...
| **4** | Full admin suite                     | ✅ v1 shipped                   | v1 complete: event CRUD + status, speakers/sponsors, agenda, activities, pre-checkin review, **attendee management, points administration, scan monitoring, role management, event overview**. Scope spec: **`CLAUDE-phase4.md`**. Reporting/exports + cross-event ranking deferred to Phase 6. |
| **5** | Networking + badges                  | ✅ v1 shipped                   | v1 complete: **networking scans** (attendee↔attendee, scanner-only payout) + **badge engine** (seeded BWAI set, `/my-badges`, scanner celebration, admin badges read). Scope spec: **`CLAUDE-phase5.md`**. Deferred: mutual credit, per-event cap, admin badge CRUD.                            |
...
> Phases 1–3 are largely built (see **Status** above); 4 is a partial; 5–6 are not started. When working on Phase 2, read **`CLAUDE-phase2.md`** for the full spec, and `PHASE2-REMAINING.md` for the short punch-list of what's left. When working on Phase 4, read **`CLAUDE-phase4.md`** for the full scope, sprint order, and migration/RLS shape. When working on Phase 5, read **`CLAUDE-phase5.md`** for the networking + badges spec. This file (`CLAUDE.md`) remains the source of truth for locked decisions and architectural rules that apply across **all** phases.
